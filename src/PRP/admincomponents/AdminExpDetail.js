...
class AdminExpDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
        }
		...
		updateTotal(n_ConfirmedAmount,n_estimatedamount,c_code,event){
			this.props.ConfirmedAmtChanged(n_ConfirmedAmount,n_estimatedamount,c_code,event)
		}
		updateTotal2(n_AdvanceAmount,n_ConfirmedAmount,c_code,event){
			this.props.AdvanceAmtChanged(n_AdvanceAmount,n_ConfirmedAmount,c_code,event)
		}
		renderTable(){
        ...
        const {c_code, c_name, n_estimatedamount, n_ApprovedAmount,n_ConfirmedAmount,n_AdvanceAmount} = item
         return (
           <tr key={index}>
             ...
             className="training-amt-input" onChange={(event) => 
              this.updateTotal2(item.n_AdvanceAmount, item.n_ConfirmedAmount, item.c_code,event)}/>
							</th>
             </>
             : 
              <>
                  ...
                  <div className="ongoing-orders-sfc mrsfcTable sfcFilterOpt">
                    <div className="prpacount">
                      ...
                      <div className="locationsfa">
                        ...
                        <div className="expense-note-det"><span className="prpexpnote">Note:</span>&nbsp; Confirmed Amount Should Be 0 or Equal To Estimated Amount.</div>
												{this.props.ConfError ? <div className="expense-note-det"><span className="prpexpnote">Confirmed Amt. By Desk Cannot Be Greater Than Estimated Amount</span></div> : null }
                        {this.props.AdvanceError ? <div className="expense-note-det"><span className="prpexpnote">Final Advance Cannot Be Greater Than Confirmed Amt. By Desk</span></div> : null }
                        </div>            				
            					</div>         
