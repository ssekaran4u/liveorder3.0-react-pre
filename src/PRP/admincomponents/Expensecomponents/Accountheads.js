...
class Accountheads extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    renderTable () {
      return this.props.Accountheads.map((item, index) => {
        const {ExpCode, ExpName, Estimated, AdvanceRequested,AdvanceReceived,n_Conf_AdvanceAmount, AdvanceConfirmed} = item
         return (
           <tr key={index}>
             ...
             {this.props.n_status <=2  ? 
              ...
              onChange={(event) => this.props.GetAdvanceConfirm(event,item.ExpCode,item.AdvanceConfirmed)}
              defaultValue={parseInt(AdvanceConfirmed)} ref={this.input} onBlur={(event) => this.props.balchanged(event)}/></th> :
              <th className="trainingamt">{AdvanceConfirmed}</th>
               }
            {this.props.n_status <= 2 ? 
            <th className="trainingamt">{AdvanceConfirmed != 0 ? Estimated - AdvanceConfirmed :  "0"}</th>:
            this.props.n_AdvReceived != '' ? <th className="trainingamt">{AdvanceReceived - AdvanceConfirmed}</th> : 
            <th className="trainingamt">{Estimated - AdvanceConfirmed}</th>}       
           </tr> 
         )
      })
    }
    render() {
      ...
      let AdvanceConfirmedTotal = this.props.Accountheads.filter(item => item.AdvanceConfirmed != "").reduce((item, currentValue) => 
        item + parseFloat(currentValue.AdvanceConfirmed), 0)

        let n_Conf_AdvanceAmountTotal = EstimatedTotal - AdvanceConfirmedTotal
        return (
            <React.Fragment>
                        ...
                        <table id="example" className="stripe row-border order-column"  >
                          ...
                          <tfoot>
                            <tr className="training-amt">  
                              ...
                              <th className="trainingamt-total">₹ {this.props.n_Balance}</th>
                            </tr>
                          </tfoot>
                        </table>
                       </div>
