...
const ConfirmEstimatedAmt =(props)=>{
    const {itemCode,rpsAmountData,advAmount,editJson,srNo,apprid,AmtEditable,nextConfirm,status} = props
        ...
        if(rpsAmountData){
           ...
           nextConfirm == "D" ?
            ...
            :
              ...
              props.getConfirmEstAmount('',editJson["estamt"],itemCode)
        }
    ...
    const getAmount=()=>{
        ...
        if(amount === '' || re.test(amount)){
            ...
            props.getConfirmEstAmount(advAmount,amount,itemCode)
        }else{
            getEstimatedAmount('')
             setValidStatus(true)
    ...
    return(
        ...
        <Form.Control type="text" 
            ...
            onChange={nextConfirm == "C" &&  status == "A" ?  ()=>getAmount()  :AmtEditable == "1" ||   status == "D" || status == "H" || status == "A" || status == "C" || status == "D" ? '' : ()=>getAmount()} 
...
export default ConfirmEstimatedAmt