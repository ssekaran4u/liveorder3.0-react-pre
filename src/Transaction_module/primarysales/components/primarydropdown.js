...
import PrimaryCopyfrom from './primarycopyfrom';
import Primarytargetable from './primarytargetable';
import SfaSpinner from "./../../../BasicComponet/sfaSpinner";


class Primarydropdown extends Component {
  constructor(props) {
    super(props);
    this.state = {
      Error: false,
  ...
  getFshqValue(fshq) {
    ...
    if (this.state.fshq.length > 0) {
      this.state.fshq.map((item) => {
        ...
        if (item.Name == fshq) {
          var targetpyedata = { "Index": "TargetType", "Data": { "FSHQType": item.Code }, }
    ...
    this.setState({ spinner: true })
      ...
      postToServer(URL_SALES, loaditemdata).then((response) => {
        ...
        if (response.status == 200) {
          ...
          this.setState({ showcopytable: true, spinner: false , copyfromValue : "-1" })
          this.CopyFromdata()
        }
      }).catch((Error) => {
        this.setState({ Error: true, Errormsg: "Error in Api At Primary Sale Target" })
  ...
  componentDidMount() {
    ...
    postToServer(URL_SALES, header).then((response) => {
      ...
      if (response.status == 200) {
        this.setState({ primaryheader: response.data.data[0].PrimSalesHeading })
      }
    }).catch((Error) => {
      this.setState({ Error: true, Errormsg: "Error in Api At Primary Sale Target" })
    })
    ...
    return (
      <React.Fragment>
        ...
        <div className="pullleft KamClaimTablesfc">
          ...
          <div className="primarysale-target1">Target On: {this.state.primaryheader}</div>


        </div>

        <div>
          <PrimaryCopyfrom showcopy={this.state.showcopytable} target={this.target} fshqcode={this.state.fshqcode} targetypecode={this.state.targetypecode}
          ...
          {this.state.showcopytable == true ?
            <Primarytargetable loaditems={this.state.loaditems} hidecopy={this.hidecopy} hideTableCopy={this.hideTableCopy}
