...
class JoinWorkOther extends Component{
    constructor(props){
        super(props)
        this.state={
             data: [],
        ...
        if(this.props.type){
            if(this.props.type=="other"){
                ...
                Object.keys(this.props.Editmodedata['workWith_multiple']).map( (next)=>{ 
                      ...
                      this.getData(k,this.props.Editmodedata['workWith_multiple'][next]["C_Name"].trim(),true,item)
                ...
                Object.keys(this.props.Editmodedata['dwr_workwith_sub_det']).map( (next)=>{ 
                    if(this.props.Editmodedata['dwr_workwith_sub_det'][next]["c_doc_code"].trim()==this.props.dsccode.trim()){ 
                      ...
                      this.getData(k,this.props.Editmodedata['dwr_workwith_sub_det'][next]["C_Name"].trim(),true,item)
                    }
...
//    }
    ...
    removeItem(id){
        ...
        this.props.Selected(selectedData)
    }
    update(dataArray){
        this.setState({
    ...
    render(){ 
       ...
       const selections = Object.keys(selectedData).reduce((p, n, i) => { 
            if (typeof(selectedData[n]) === "string") { 
                ...
                p.push(
                        <div key={n} className="selectedDropdown"> {name.toLowerCase()+"("+desg+")"}
...
export default connect(mapStateToProps, mapDispatchToProps)(JoinWorkOther)
