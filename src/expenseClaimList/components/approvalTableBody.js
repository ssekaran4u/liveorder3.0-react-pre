...
class ApprovalTableBody extends Component {
    constructor(props) {
        super(props);
        this.state = {
            showPopUp: false,
                ...
                postToServer(URL_EXPENSE_CLAIM, data).then((Result) => {
                    if (Result.data.Status == 'Success') {
                        if (Result.data.data.length) {
                            this.props.history.push("/expensemcr/" + ans[0].C_Code + "/" + "approval/" + dtt + "/" + ans[0].C_Name)
                        }
                        else {
    ...
    onDate(date, no) {
        ...
        postToServer(URL_EXPENSE_CLAIM, data).then((Result) => {
            if (Result.data.Status == 'Success') {
                if (Result.data.data.length) {
                    this.props.history.push("/expensedcrlist/" + no + "/" + "approval/" + date)
                }
                else {
    ...
    render() {
        ...
        let approvedFlags = this.props.approvedFlags;
        return (
            <React.Fragment>
                <Modal
...
export default withRouter(ApprovalTableBody);