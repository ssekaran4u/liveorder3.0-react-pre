...
class ReConfirmationTable extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isFull: false,
        ...
        postToServer(URL_EXPENSE_CLAIM, ben_details).then((Result) => {
            if (Result.data.Status == 'Success') {
                if (Result.data.flag[0].Status == "1" || Result.data.flag[0].Status == "2") {
                    if (Result.data.flag[0].AllowReconfirmation == "1") {
                        this.setState({
                            disable: false,
    ...
    render() {
        ...
        var body = []
        if (this.props.reConfirmationList != undefined) {
            this.props.reConfirmationList.map((list) => {
                body.push({
                    reportNo: list["SrNo"],
                    empCode: list["EmpCode"],
                    empName: list["EmployeeName"],
                    designation: list["Designation"],
                    frDate: list["FrDate"],
                    toDate: list["ToDate"],
                    region: list["Region"],
                    area: list["Area"],
                    division: list["DivisionName"],
                    deskName: list["DeskName"],
                    approvedOn: list["ApprovedDate"],
                    confirmedOn: list["ConfirmedDate"],
            ...
            body.map((res) => {
                if (res.reportNo != "") {
                    res.reportNo =
                        <div>
                            <Link to={"/expenseclaimentry/" + "00/" + "reconfirmation/" + res.reportNo}>
                                {res.reportNo}
                            </Link>
                        </div>
                }
...
export default ReConfirmationTable;