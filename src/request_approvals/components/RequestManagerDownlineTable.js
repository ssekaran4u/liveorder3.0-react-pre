...
class RequestManagerDownlineTable extends Datatable {
    
    // handleStatus(e,datatype,code){
    //     const _this=this
    //    let filteredRequest=[]
    ...
    //        filter = code
    //    }
    //    this.props.filterRequestList(status,filter,'','')
     
    // }
    // handleSelect(range){ console.log("range",range)
    //     const startdate = convert(range.startDate._d);
    //     const enddate = convert(range.endDate._d);
    // //    // this.props.filterDate(startdate,enddate)
    //     this.props.filterRequestList('','',startdate,enddate)
    // }
    handleStatus(status){
       
       this.props.filterStatusList(status)
     
    }
    handleFilter(filter){
       
       this.props.filterTypeList(filter)
    ...
    render() {
        ...
        return (
            <div className="managerReq">
                ...
                {/* <div className="mangerapprovfiltersBtn">
                            <button type="button" className="managerapproval_button">Approve All Selected</button>
                        </div> */}
                    <div>
                     ...
                     <Dropdown.Menu>
                        <Dropdown.Item href="#/action-1" >
                            ...
                            {this.props.requestTypeList ? this.props.requestTypeList.map((item,index)=>(
                                 <div key={index} className="statussubmenu" onClick={(() => this.handleFilter(item.Code)).bind(this)} >{item.Name}</div>    
                               )):''}   
                            </div>
                        </Dropdown.Item>
...
export default RequestManagerDownlineTable;